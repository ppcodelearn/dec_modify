...
class DEC(object):
        ...
        if y is not None:
            class PrintACC(callbacks.Callback):
                ...
                def on_epoch_end(self, epoch, logs=None):
                    ...
                    # plot the cluster
                    plt.scatter(features[:,0],features[:,1],c=self.y)
    ...
    def fit(self, x, y=None, maxiter=2e4, batch_size=256, tol=1e-3,
        ...
        # plot the cluster
        plt.scatter(self.encoder.predict(x)[:,0],self.encoder.predict(x)[:,1],c=y)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', default='mnist',
                        choices=['mnist', 'fmnist', 'usps', 'reuters10k', 'stl', 'manuel'])
    ...
    if args.dataset == 'manuel' or args.dataset == 'manuel':
        update_interval = 140
        pretrain_epochs = 300
        init = VarianceScaling(scale=1. / 3., mode='fan_in',
                               distribution='uniform')  # [-limit, limit], limit=sqrt(1./fan_in)
