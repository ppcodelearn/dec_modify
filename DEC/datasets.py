...
def load_manuel():
    manuel_dir = r'C:\Users\phili\techlabs\eth_sms\Sensordata_Manuel' 
    manuel_list = os.listdir(manuel_dir)
    data = []
    for index in range(0, len(manuel_list)): #start from new dataset
        data_manuel = manuel_list[index]
        data.append(os.path.join(manuel_dir, data_manuel))
    x = np.load(data[0])
    x = x.reshape((x.shape[0], -1))
    y = np.load(data[1])
    x = np.divide(x, x.max())
    print('Manuel samples', x.shape)
    return x, y
